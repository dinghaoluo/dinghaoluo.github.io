...
<div class="photos-intro">
  <p>Places I have lived and places I have passed through, organised by where rather than when. Click a pin on the map or scroll down.</p>
</div>
...
<div id="photo-map-panel" class="photo-map-panel" aria-label="Regional maps for the photo atlas">
    ...
    <section id="{{ section.id }}" class="photo-section photo-section--{{ section.accent | default: 'clay' }}{% if section.layout %} photo-section--{{ section.layout }}{% endif %}">
      ...
      <div class="photo-cluster">
        {% for photo in section.photos %}
          ...
          {% endif %}
          <figure{% if photo.anchor %} id="{{ photo.anchor }}"{% endif %} class="photo-frame photo-frame--{{ photo.slot | default: 'wide' }}{% if photo.mobile_half %} photo-frame--mobile-half{% endif %}" style="--photo-ratio: {{ photo.width }} / {{ photo.height }};">
