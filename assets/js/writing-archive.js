...
(function () {
  ...
  function initWritingArchive() {
    ...
    function movePill(button) {
      ListUtils.movePill(strip, pill, button);
    }

    function restoreDateOrder() {
      entries = ListUtils.restoreOrder(entries, archive, 'originalIndex');
      shuffled = false;
    }
    ...
    function totalPages(count) {
      return ListUtils.totalPages(count, PAGE_SIZE);
    }
    ...
    function handlePageInput(input) {
      ListUtils.handlePageInput(input, function () {
        return totalPages(matchedEntries().length);
      }, goToPage);
    }
