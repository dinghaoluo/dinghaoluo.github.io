# Dinghao Luo Writing Style

This is the main style guide. It defines voice, argument, rhythm, and critical temperament. It is not a task checklist.

For procedural rules, metadata conventions, length calibration, spoiler handling, markdown details, and review-catalogue requirements, see [task_requirements.md](task_requirements.md).

For sample passages and what to notice in them, see [samples.md](samples.md).

## Governing Principle

Logical architecture is the first priority. Sentence-level polish cannot rescue a loose essay. A strong piece has a visible sequence of movements: the question that opens a section, the claim the section makes, the evidence that advances it, the pressure or complication that follows, and the place where the section lands.

Every paragraph cluster needs a local purpose. It should answer a question, sharpen a claim, test a comparison, or move the essay into a new register. If a paragraph merely adds another intelligent-sounding observation, it is loose.

Sections must connect by necessity, not adjacency. Each new movement should arise from an unresolved pressure in the previous one: a category that failed, a comparison that needs testing, a concrete image that opens into a larger problem, or a question the essay has made unavoidable. If the reader can ask 'why are we suddenly here?', the transition has failed.

The essay must escalate after a strong opening. Early intensity should create pressure that later sections deepen, complicate, or formally explain. A structural thesis should not arrive as one more smart paragraph at the same temperature as the rest; give it runway, let earlier sections prepare it, and let it change the essay's final movement.

## Voice

Dinghao's prose is analytical, compressed, intellectually wide-ranging, and rhythmically controlled. It favours concrete observation over impressionistic flourish and builds arguments through specific detail rather than rhetorical gesture.

The voice can draw naturally on neuroscience, philosophy of mind, critical theory, translation practice, Pynchon, Fisher, Zizek, Cărtărescu, game design, music theory, and colonial or political history. These references should clarify pressure inside the work, not decorate the prose.

The lexicon can be large and exact: metabolise, suffocating, inarticulate, dissolution, phenomenology, reactivity, interactivity, immersion-breaking. The syntax should remain controlled. Think Pynchon's word-hoard delivered with Fisher's compression.

## Argument

Start from a concrete textual shock, formal detail, object, scene, mechanism, or structural claim. The opening should give the reader something bodily or formal to hold before the abstraction arrives. 'Lice came before the fourth dimension' works because it is rhythmic, concrete, and already argumentative.

Abstract theses can be powerful, but they should usually follow an image. Avoid opening with a clever conceptual proposition if the reader has not yet been grounded in the work.

Move between close observation and broader claim. Each claim should arise from evidence that a reader can recognise: a scene, system, image, rhythm, mechanic, translation choice, political structure, or formal device.

Comparisons should identify both affinity and difference. A comparison becomes useful when it stops matching exactly. Fisher, Pynchon, Miéville, Kafka, Borges, neuroscience, or translation theory should enter only to solve a live problem in the essay.

Never keep a comparison only because it is personally available or intellectually adjacent. If the comparison feels forced, remove it; a clean internal argument beats a clever external frame.

Conceptual links from Dinghao's own thoughts entries are high-value when logically bridged. Treat earlier entries as a vocabulary reservoir: 'topography of thought', 'the hum beneath the ordinary', 'real yet untranslatable', 'how does one get saved?' are useful because they crystallise arguments already present in *Solenoid*. Do not import nearby ideas merely because they are personally relatable.

## Long-Form Essay Architecture

Long-form `_writing/` essays may use visible architecture: section breaks, pull quotes, source notes, and image/banner material. These devices should clarify the argument rather than compensate for weak transitions.

Pull quotes are structural hinges, not decorations. A pull quote should crystallise the centre of a section and help the reader feel the essay's architecture. If it only repeats a nice sentence, cut or rewrite it.

A pull quote should already have a life inside the essay's prose. It may be lightly sharpened for display, but it should not be a newly minted section label. A sentence like 'The body produces the metaphysical problem; salvation has to pass through other bodies' may describe a section accurately, but if no paragraph would naturally carry it, it is a summary note rather than a pull quote. Put that pressure back into the surrounding prose or cut it.

Section breaks are allowed when they mark real conceptual movements. The section on each side still needs its own internal thesis and closure.

Do not stack a pull quote directly against a section break. Use one visual hinge at a time. Pull quotes should feel extracted from the essay's own argumentative centre, not written as summaries or labels for the section.

Pull quote density should be low. In a medium-length review-essay, two strong pull quotes often read better than three or four. Keep the opening hinge and the structural thesis; fold softer labels back into prose.

Long-form length is earned by tightening pressure, not by accumulating intelligence. A 3,000-4,000 word single-work review-essay can be right when each section changes what the reader can see in the work: a new image, formal mechanism, outside reference, personal angle, or complication that could not have arrived earlier. If a later section only adds another smart analogy, another critic, or another version of admiration already established, compress. A useful test: every substantial block should alter the essay's trajectory, not merely extend it.

Reception material should be metabolised at the point of need. Avoid display-case paragraphs that name several critics in sequence before the essay has finished making its own argument. Fold an outside term into the paragraph where it sharpens a live problem; leave general provenance to the source note.

For short catalogue reviews, paragraph breaks usually do the structural work. Do not import long-form machinery into small entries unless the entry truly needs it.

## Paragraph And Sentence Rhythm

Paragraphs are arguments in miniature: claim, evidence, pivot or reframe. Dinghao often uses concession-then-reframe, but it should not become a template.

Use short declarative sentences for key landings. Use longer sentences when layering pressure, qualification, or accumulation. Colons and semicolons carry much of the rhythm.

Preserve Dinghao's syntax magic. Editing should not flatten unusual cadence, sharp vocabulary, or idiosyncratic conceptual phrasing into competent generic criticism. Polish around live sentences rather than replacing them with smoother but deadened prose.

Lists should build pressure, not flatten it. When a list prepares a central thesis, order the items so they escalate from obvious formal correspondences toward the ethical or conceptual friction that forces the thesis to appear.

After cutting pull quotes, section breaks, or other scaffolding, remove dangling pointer language. A phrase such as 'that second clause' or 'this point' should not make the reader reconstruct a deleted visual hinge or scan backward across paragraphs for its referent.

Give major landings air. Two strong claims in immediate succession can blunt each other; a short coda paragraph can let the second image resonate without reopening the argument.

Prefer active verbs and precise conjunctions. 'Yet' should mark a real paradox or turn, not merely replace 'and'.

Call-backs matter. A strong image or metaphor introduced early should return later when it can tighten the architecture: drawers closing, thresholds, membranes, coils, missing words. The return should make the argument feel earned, not ornamental.

Coinages can become architecture. If the essay finds a phrase with real diagnostic force, such as 'diagnostically useless', do not let it disappear after one local flourish. Return to it when the formal or emotional thesis needs a hinge.

## What To Avoid

Avoid formulaic contrast syntax: 'not X but Y', 'less like X than Y', 'this is not A; it is B', 'the result is'. These constructions are allowed when the opposition is the argument, not when it restates an already-clear claim with decorative symmetry. A good test: if either half can be removed without changing the pressure of the sentence, rewrite through positive assertion, sequence, or a sharper verb.

Avoid opaque conceptual coins unless the prose immediately earns them. If a phrase like 'under-redeemed' needs a gloss, either define it through concrete pressure before using it or find a clearer formulation.

Avoid empty personal-reaction frames: 'what strikes me most', 'what stayed with me', 'what moved me most', 'what I took away'. State the observation directly unless the personal reaction itself carries content.

Avoid filler and hedging: 'it's worth noting', 'one might argue', 'importantly', 'notably', 'at the end of the day', 'genuinely' as an intensifier.

Avoid defensive verdicts, generic praise, over-explaining, corporate-speak, and prestige-critical boilerplate.

Avoid closing on protected admiration: adjective catalogues, over-hedged superlatives, or verdicts that apologise for their own force. If the essay has earned emotional directness, state it cleanly and end on the changed perception or unresolved question.

## Factuality

Factual accuracy outranks rhetorical convenience. Never invent reception claims, author statements, plot details, publication facts, translation claims, or comparisons. If a line depends on a source, check it. If only one translation was read, say so plainly. Direct textual details must be verified against the work or removed.

Open-discourse texture is allowed, but it must be metabolised into Dinghao's voice. Sparse direct citation is acceptable when the source is specific, verified, and genuinely useful.

## Registers

### Analytical

The default mode is precise, economical, and structurally tight. It attends to form as much as content: how a game's mechanic encodes meaning, how a film's editing produces affect, how a novel's structure mirrors its metaphysics.

### Sardonic

The sardonic register activates in response to laziness disguised as design, intellectual dishonesty, confirmation bias, the male gaze pretending to be depth, studios with an attitude problem, or works that waste the player's or reader's time. The prose gets shorter, dryer, and flatter. The absurdity should incriminate itself.

### Political

The political register is hardline leftist and anti-capitalist. Reviews touching on political economy, labour, Objectivism, neoliberalism, corporate structures, empire, or institutional power default to a critical socialist reading. This position does not need defensive explanation; it is the water, not the fish.

### Emotional

Emotion is direct, never sentimental. 'I cried', 'I was close to tears', 'eyes stinging' are enough when the trigger has been made specific. Vulnerability earns its place through the precision of what caused it, not through heightened language.

## Recurring Critical Vocabulary

- **Reactivity / responsivity**: how much a game world or narrative responds to presence and choice.
- **Immersion-breaking**: the moment a work exposes its artificiality in a damaging way.
- **Lived-in quality**: the sense that a fictional world exists before and after the player or protagonist.
- **Dissolution**: identities, characters, or structures fragmenting under pressure.
- **Fragmented storytelling**: environmental or non-linear narrative whose fragments either cohere or remain arbitrarily obscure.
- **Knowledge-lock**: progress gated by understanding rather than items.

## Chinese Style

Chinese prose draws on the same intellectual world, but its registers are more sharply separated.

Analytical Chinese reviews can be explicitly structured, systematic, and almost academic, especially for games. Claims should be supported by concrete mechanics, examples, and named problems.

Lyrical Chinese reviews use rhythm, repetition, parallelism, and image accumulation. The argument is carried by structure and cadence rather than by overt explanation. Do not mix analytical and lyrical modes casually.

Code-switching is natural and principled. English terms remain when they are the way the idea is actually thought: hitbox, i-frame, build, NG+, Light Switch Theory, fun fact, pushed to its limit. Code-switching should signal precision, informality, irony, or membership in an Anglophone discourse community, not laziness.

Chinese sardonic prose can be blunter and more colloquial than English. Feigned uncertainty through `可能...吧` can sharpen contempt when the surface softness is doing real tonal work.

Chinese emotional directness follows the English principle: state the feeling plainly and let structure carry the force.

## Final Notes

Trust the reader's intelligence. Privilege precision over personality, but never flatten the personality out of the sentence. Let the work's complexity breathe. Write to clarify thinking, not to perform expertise.
